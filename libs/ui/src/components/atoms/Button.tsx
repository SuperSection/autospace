...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantClass = variantColor[variant][color];
  //   variant === 'text' ? sizes.none :
  const sizeClass = sizes[size];

  const fwClass = fullWidth && 'w-full';
  const disClass = (disabled || loading) && 'opacity-60 cursor-auto';

  return (
    <button
      type={type}
      disabled={disabled || loading}
      className={`rounded relative font-medium ${sizeClass} ${fwClass} ${variantClass} ${disClass}  ${className} `}
      {...props}
    >
      {loading ? (
        <>
